...
def get_visitor_post(pageid):
    fburl = "https://graph.facebook.com/" + pageid + "/feed?fields=shares,picture,likes,message,created_time,link,target,name,id,feed_targeting,object_id,parent_id,from,description,type&limit=100&access_token=181560758911664|5b0874a270777180de4089788624ca97"
    fburln = fburl    
    df1 = []
    while True:
        ...
        for data in json_post['data']:
           if data.get('target'):           
               ...
               if data.get('name'):
                   ...
                   postname = ""                 
               if data.get('likes'):
                   count_likes = len(data['likes']['data'])       
               else:
                   count_likes = 0  
               ...
               df1.append({'target_id' : targetid, 'target_name' : targetname, 'post_id' : postid, 'post_link' : postlink, 'post_type' : posttype, 'post_name' : postname, 'user_id' : userid, 'user_name' : username, 'message' : message, 'date' : date, 'count_likes' : count_likes, 'count_shares' : count_shares})
...
def get_tagged_post(pageid):
    #get tagged post
    fburl1 = "https://graph.facebook.com/" + pageid + "/tagged?fields=shares,from,likes,message,tagged_time,id,link,type&limit=100&access_token=181560758911664|5b0874a270777180de4089788624ca97"
    fburltag = fburl1
    ...
    while True:
        ...
        for data in json_post['data']:
            ...
            date = date[0]
            date = datetime.datetime.strptime(date, '%Y-%m-%d').strftime('%d-%m-%Y')
            df2.append({'post_id' : postid, 'post_link' : postlink , 'post_type' : posttype , 'user_id' : userid, 'user_name' : username, 'message' : message, 'date' : date, 'count_likes' : count_likes, 'count_shares' : count_shares})
        if json_post.get('paging'):
...
#get post by page
            if not data.get('target'):
                ...
                if data.get('comments'):
                    count_likes = len(data['comments']['data'])       
                else:
                    count_likes = 0
                ...
                date = date[0]
                date = datetime.datetime.strptime(date, '%Y-%m-%d').strftime('%d-%m-%Y')
                df.append({'post_id' : postid, 'post_link' : postlink, 'post_type' : posttype, 'post_name' : postname, 'user_id' : userid, 'user_name' : username, 'message' : message, 'date' : date, 'count_likes' : count_likes, 'count_shares' : count_shares})
...
df_page_post.to_csv('E:\IDXP\FWD\Facebook_FWD_page_post.csv', sep='`', encoding='utf-8')
df_visitor_post.to_csv('E:\IDXP\FWD\Facebook_FWD_visitor_post.csv', sep='`', encoding='utf-8')
df_tagged_post.to_csv('E:\IDXP\FWD\Facebook_FWD_tagged_post.csv', sep='`', encoding='utf-8')
df_review_post.to_csv('E:\IDXP\FWD\Facebook_FWD_review_post.csv', sep='`', encoding='utf-8')

df_tagged_post_comments, df_tagged_post_replies = iterate_post(df_tagged_post['post_id'])
df_tagged_post_comments.to_csv('E:\IDXP\FWD\Facebook_FWD_tagged_post(comments).csv', sep='`', encoding='utf-8')
#df_tagged_post_likes.to_csv('E:\IDXP\FWD\Facebook_FWD_tagged_post(likes).csv', sep='`', encoding='utf-8')
df_tagged_post_replies.to_csv('E:\IDXP\FWD\Facebook_FWD_tagged_post(replies).csv', sep='`', encoding='utf-8')

df_page_post_comments, df_page_post_replies = iterate_post(df_page_post['post_id'])
df_page_post_comments.to_csv('E:\IDXP\FWD\Facebook_FWD_page_post(comments).csv', sep='`', encoding='utf-8')
#df_page_post_likes.to_csv('E:\IDXP\FWD\Facebook_FWD_page_post(likes).csv', sep='`', encoding='utf-8')
df_page_post_replies.to_csv('E:\IDXP\FWD\Facebook_FWD_page_post(replies).csv', sep='`', encoding='utf-8')


df_visitor_post_comments, df_visitor_post_replies = iterate_post(df_visitor_post['post_id'])
df_visitor_post_comments.to_csv('E:\IDXP\FWD\Facebook_FWD_visitor_post(comments).csv', sep='`', encoding='utf-8')
#df_visitor_post_likes.to_csv('E:\IDXP\FWD\Facebook_FWD_visitor_post(likes).csv', sep='`', encoding='utf-8')
df_visitor_post_replies.to_csv('E:\IDXP\FWD\Facebook_FWD_visitor_post(replies).csv', sep='`', encoding='utf-8')
