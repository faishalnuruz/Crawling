...
def iterate_post(json_posts):
    ...
    for row in json_posts['data']['user']['edge_owner_to_timeline_media']['edges']:
        ...
        #df_comments = get_comments(id_owner, code_post, count_comments, id_fetchcomment)
#        df = df.append(df_comments)
        df2.append({'id_owner': id_owner,'id_post': id_post, 'link_post' : code_post, 'type_post': type_post, 'count_comments': count_comments, 'count_likes': count_likes, 'timestamp_post': timestamp_post, 'desc_post' : desc_post })
#            break;
#        print('yes')
#        i = i + 1
        i = i + 1
...
def main_crawl():
    ...
    dffinal2 = iterate_post(posts)
    outfileurl = "D:\instagram_delim.csv"    

    dffinal.to_csv(outfileurl, sep='`', encoding='utf-8', index=False)
    df2 = pd.read_csv("D:/instagram.csv", sep="`")
